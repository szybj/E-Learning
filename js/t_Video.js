...
var fu = new FileUpload("uploadVideo", "idVideo", { 
    ...
    onIni: function(){
        ...
        AddList(arrRows);
        //设置按钮
        $("idBtnuploadVideo").disabled = $("idBtndelVideo").disabled = this.Files.length <= 0;
    }
});

$("idBtnuploadVideo").onclick = function(){
    //显示文件列表
    var arrRows = [];
    ...
    $("idProcessVideo").style.display = "";
    $("idMsgVideo").innerHTML = "正在添加文件到您的网盘中，请稍候……<br />有可能因为网络问题，出现程序长时间无响应，请点击“<a href='?'><font color='red'>取消</font></a>”重新上传文件";
...
function AddList(rows){
    //根据数组来添加列表
    var FileList = $("idFileListVideo"), oFragment = document.createDocumentFragment();
    //用文档碎片保存列表
...
$("idLimitVideo").innerHTML = fu.Limit;

$("idExtVideo").innerHTML = fu.ExtIn.join("，");

$("idBtndelVideo").onclick = function(){ fu.Clear(); }
