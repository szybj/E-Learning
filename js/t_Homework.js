...
var fu = new FileUpload("uploadHomework", "idHomework", { 
    ...
    onIni: function(){
        ...
        if(this.Files.length){
            ...
            Each(this.Files, function(o){
                ...
                a.innerHTML = "取消"; a.href = "javascript:void(0);";
        ...
        AddList(arrRows);
        //设置按钮
        $("idBtnuploadHomework").disabled = $("idBtndelHomework").disabled = this.Files.length <= 0;
    }
});

$("idBtnuploadHomework").onclick = function(){
    //显示文件列表
    var arrRows = [];
    ...
    $("idProcessHomework").style.display = "";
    $("idMsgHomework").innerHTML = "正在添加文件到您的网盘中，请稍候……<br />有可能因为网络问题，出现程序长时间无响应，请点击“<a href='?'><font color='red'>取消</font></a>”重新上传文件";
...
function AddList(rows){
    //根据数组来添加列表
    var FileList = $("idFileListHomework"), oFragment = document.createDocumentFragment();
    //用文档碎片保存列表
...
$("idLimitHomework").innerHTML = fu.Limit;

$("idExtHomework").innerHTML = fu.ExtIn.join("，");

$("idBtndelHomework").onclick = function(){ fu.Clear(); }
