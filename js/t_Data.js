...
FileUpload.prototype = {
  ...
  SetOptions: function(options) {
    this.options = {//默认值
        ...
        ExtIn:      ["doc","docx","rar","ppt","pptx","zip"],//允许后缀名
...
var fu = new FileUpload("uploadData", "idData", { 
    ...
    onIni: function(){
        ...
        AddList(arrRows);
        //设置按钮
        $("idBtnuploadData").disabled = $("idBtndelData").disabled = this.Files.length <= 0;
    }
});

$("idBtnuploadData").onclick = function(){
    //显示文件列表
    var arrRows = [];
    ...
    $("idProcessData").style.display = "";
    $("idMsgData").innerHTML = "正在添加文件到您的网盘中，请稍候……<br />有可能因为网络问题，出现程序长时间无响应，请点击“<a href='?'><font color='red'>取消</font></a>”重新上传文件";
...
function AddList(rows){
    //根据数组来添加列表
    var FileList = $("idFileListData"), oFragment = document.createDocumentFragment();
    //用文档碎片保存列表
...
$("idLimitData").innerHTML = fu.Limit;

$("idExtData").innerHTML = fu.ExtIn.join("，");

$("idBtndelData").onclick = function(){ fu.Clear(); }
